...
/*Title:HappyChat聊天系统用户注册程序 详情见图片3
 *@version 1.0
 */
...
public class Register extends JFrame  implements ActionListener
{
	...
	public void register()
	{
		...
		try
		{
			...
			// toServer.getOutputStream(): 套接字的输出流
			// 将套接字的输出流用一个ObjectOutputStream保存起来
			// 先创建流再写值 写值见下句
		    ObjectOutputStream streamToServer=new ObjectOutputStream (toServer.getOutputStream());					
		    //写客户详细资料到服务器socket
			// 可以不用强制转换 data本身就是Register_Customer类型
			// 用writeObject所写入的类必须要实现serilizebal接口
